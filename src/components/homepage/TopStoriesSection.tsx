  ...
  return (
    <section className="md:bg-[#f7fafc] md:rounded-lg md:border md:border-slate-200/20 md:p-6">
      <div className="max-w-[917px] md:max-w-none">
        ...
        {/* Desktop Layout */}
        <div className="hidden md:grid grid-cols-1 lg:grid-cols-[65%_35%] gap-6 mt-6">
          {/* Large Story */}
          ...
          <div className="bg-white rounded border p-6 space-y-6">
            {sideArticles.map((article, index) => (
              <article key={article.id || index} className="space-y-1">
                ...
                <Link href={`/news-details/${article.id}`}>
                  <h4 className="text-lg leading-tight font-medium hover:text-[#cc0000] transition-colors">{article.title}</h4>
                </Link>
              </article>
            ))}
          </div>
